...
* Full name: `\Saggre\WordPress\Repository\Config\PluginDownloadClientConfig`
...
```php
public __construct(string $baseUrl = 'https://downloads.wordpress.org', string $userAgent = 'wordpress-org-repository-php-wrapper/' . self::CLIENT_VERSION): mixed
```
...
|--------------|------------|------------------------------------------------|
| `$baseUrl`   | **string** | The base URL for the plugin distribution host. |
